...
console.log('[zamok-api-demo] Welcome to the Zamok API Demo test page.');
// console.log('[zamok-api-demo] Environment:', process.env.NODE_ENV);
...
zamok.ready
  .then(() => {
    ...
    const dispatch = () => {
      ...
      console.log('[zamok-api-demo] Dispatching action to Zamok...', { action });
    ...
    zamok.getState().then((state) => {
      console.log('[zamok-api-demo] latest state from the kiosk:', state);
      initialState = state;
      renderStateTree(initialState, 'json-container');
    });
  })
  .catch((error) => console.error('[zamok-api-demo] Unable to connect to Zamok.', { error }));
